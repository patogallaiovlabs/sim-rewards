...
current_block_reward_percentage = 0.05  # 10% for current block miner
base_fee = 0.001
spike_fee = 1.0
...
# Configure which pools are strategic (set to empty set for all honest)
...
def run_hashrate_sweep_single_run(strategic_hashrate):
    ...
    pool_hashrates = {
        ...
        1: 1.0 - strategic_hashrate,   # Honest pool
    }
    ...
    honest_reward = honest_rewards[0]  # Pool 0 is strategic
    ...
    return strategic_hashrate, improvement
...
def run_multiple_simulations(num_runs=100):
    ...
    all_results = {h: [] for h in hashrates}
    ...
    for run in range(num_runs):
        ...
        for strategic_hashrate in hashrates:
            _, improvement = run_hashrate_sweep_single_run(strategic_hashrate)
            all_results[strategic_hashrate].append(improvement)
    
    # Calculate averages and statistics
    avg_results = []
    for hashrate in hashrates:
        improvements = all_results[hashrate]
        ...
        print(f"Hashrate {hashrate:.1f}: Avg improvement {avg_improvement:+.2f}% ± {std_improvement:.2f}%")
    
    return avg_results

def run_current_block_reward_comparison():
    """Compare different current block reward percentages"""
    print("\n=== CURRENT BLOCK REWARD PERCENTAGE COMPARISON ===")
    ...
    for reward_pct in reward_percentages:
        ...
        # Run honest and strategic simulations with this percentage
        honest_rewards, honest_metrics = deterministic_simulation("honest", fixed_miner_sequence, float(reward_pct))
        strategic_rewards, strategic_metrics = deterministic_simulation("strategic", fixed_miner_sequence, float(reward_pct))
        
        # Calculate improvements for each pool
        pool_results = {}
        ...
        for pool_id in pool_hashrates.keys():
            ...
            print(f"    Honest reward: {pool_results[pool_id]['honest_reward']:.2f}")
            print(f"    Strategic reward: {pool_results[pool_id]['strategic_reward']:.2f}")
            print(f"    Improvement: {pool_results[pool_id]['improvement']:+.2f}%")
    
    return comparison_results

def run_hashrate_sweep_with_reward_percentage(strategic_hashrate, reward_pct):
    """Run a single simulation for a given hashrate and reward percentage"""
    # Update pool hashrates
    pool_hashrates = {
        0: strategic_hashrate,    # Strategic pool
        1: 1.0 - strategic_hashrate,   # Honest pool
    ...
    # Run simulations with the specified reward percentage
    honest_rewards, honest_metrics = deterministic_simulation("honest", fixed_miner_sequence, float(reward_pct))
    strategic_rewards, strategic_metrics = deterministic_simulation("strategic", fixed_miner_sequence, float(reward_pct))
        ...
        for _ in range(blocks):
            ...
            for pool_id, hashrate in pool_hashrates_temp.items():
                ...
                if rand < cumulative:
                    ...
                    break
        miner_sequences[strategic_hashrate] = sequence
    ...
    for reward_pct in reward_percentages:
        ...
        for run in range(RUNS):
            ...
            for strategic_hashrate in hashrates:
                ...
                sequence = miner_sequences[strategic_hashrate]
                ...
                all_results[reward_pct][strategic_hashrate].append(improvement)
...
# Run current block reward comparison
comparison_results = run_current_block_reward_comparison()
...
plt.ylabel('Strategic Pool Improvement (%)')
plt.title('Strategic Pool Improvement vs Hashrate for Different Current Block Reward Percentages')
