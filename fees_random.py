...
def deterministic_simulation(strategy: str, miner_sequence):
    ...
    for block in range(blocks):
        ...
        # Generate random fee for current block
        if block in spike_positions:
            # Spike fee - higher random fee
            fee += random.uniform(spike_fee_min, spike_fee_max)
        else:
            # Base fee - lower random fee
...
def run_multiple_simulations(num_runs=100):
    ...
    for hashrate in hashrates:
        ...
        honest_improvements = all_honest_results[hashrate]
        
        avg_strategic_improvement = np.mean(strategic_improvements)
        std_strategic_improvement = np.std(strategic_improvements)
        avg_honest_improvement = np.mean(honest_improvements)
        std_honest_improvement = np.std(honest_improvements)
        
        # Calculate average percentages
        avg_honest_pct = {
            pool_id: np.mean(all_honest_percentages[hashrate][pool_id]) 
            for pool_id in [0, 1]
        }
        avg_strategic_pct = {
        ...
        avg_results.append({
            ...
            'avg_honest_improvement': avg_honest_improvement,
            'std_honest_improvement': std_honest_improvement,
            'min_honest_improvement': min(honest_improvements),
            'max_honest_improvement': max(honest_improvements),
            'avg_honest_percentages': avg_honest_pct,
            'avg_strategic_percentages': avg_strategic_pct
        })
        
        print(f"Hashrate {hashrate:.1f}: Strategic avg {avg_strategic_improvement:+.2f}% ± {std_strategic_improvement:.2f}%, Honest avg {avg_honest_improvement:+.2f}% ± {std_honest_improvement:.2f}%")
...
# Print results
print("=== SCENARIO COMPARISON (RANDOM FEES) ===")
print(f"Scenario 1: All pools honest")
print(f"Scenario 2: Pool {strategic_pools} strategic, others honest")
print()
...
print(f"Fee ranges: Base fees {base_fee_min}-{base_fee_max}, Spike fees {spike_fee_min}-{spike_fee_max}")
print()
...
avg_honest_improvements = [r['avg_honest_improvement'] for r in avg_results]
std_honest_improvements = [r['std_honest_improvement'] for r in avg_results]
...
# Plot honest pool line
plt.plot(hashrates, avg_honest_improvements, 'ro-', linewidth=2, markersize=8, label='Honest Pool Impact')
...
plt.errorbar(hashrates, avg_honest_improvements, yerr=std_honest_improvements, fmt='none', capsize=5, alpha=0.7, color='red')

plt.xlabel('Strategic Pool Hashrate')
plt.ylabel('Average Reward Change (%)')
plt.title('Strategic vs Honest Pool Reward Impact (Random Fees, 100 runs)')
...
# Calculate the difference (strategic gain - honest loss)
differences = [s - h for s, h in zip(avg_strategic_improvements, avg_honest_improvements)]
std_differences = [np.sqrt(s**2 + h**2) for s, h in zip(std_strategic_improvements, std_honest_improvements)]

plt.plot(hashrates, differences, 'go-', linewidth=2, markersize=8, label='Net Impact (Strategic - Honest)')
...
plt.title('Net Impact of Strategic Behavior (Random Fees, 100 runs)')
plt.grid(True, alpha=0.3)
plt.xticks(hashrates)
...
# Print summary
print("\n=== SUMMARY (100 runs) ===")
print("Hashrate | Strategic Impact | Honest Impact | Net Impact")
print("-" * 70)
for r in avg_results:
    net_impact = r['avg_strategic_improvement'] - r['avg_honest_improvement']
    print(f"{r['hashrate']:.1f}     | {r['avg_strategic_improvement']:+.2f}% ± {r['std_strategic_improvement']:.2f}% | {r['avg_honest_improvement']:+.2f}% ± {r['std_honest_improvement']:.2f}% | {net_impact:+.2f}%")

print("\n=== REWARD DISTRIBUTION SUMMARY (100 runs) ===")
print("Hashrate | All Honest: Pool0 | Pool1 | Strategic: Pool0 | Pool1")
print("-" * 80)
for r in avg_results:
    honest_p0 = r['avg_honest_percentages'][0]
    honest_p1 = r['avg_honest_percentages'][1]
    strategic_p0 = r['avg_strategic_percentages'][0]
    strategic_p1 = r['avg_strategic_percentages'][1]
    print(f"{r['hashrate']:.1f}     | {honest_p0:.1f}%        | {honest_p1:.1f}% | {strategic_p0:.1f}%        | {strategic_p1:.1f}%")

# Create reward distribution comparison plots
plt.figure(figsize=(15, 10))

# Subplot 1: Honest scenario percentages
plt.subplot(2, 2, 1)
honest_pct_pool0 = [r['avg_honest_percentages'][0] for r in avg_results]
honest_pct_pool1 = [r['avg_honest_percentages'][1] for r in avg_results]

plt.plot(hashrates, honest_pct_pool0, 'bo-', linewidth=2, markersize=6, label='Pool 0 (Strategic)')
plt.plot(hashrates, honest_pct_pool1, 'ro-', linewidth=2, markersize=6, label='Pool 1 (Honest)')

plt.xlabel('Strategic Pool Hashrate')
plt.ylabel('Reward Percentage (%)')
plt.title('Reward Distribution - All Honest Scenario (Random Fees)')
plt.grid(True, alpha=0.3)
plt.xticks(hashrates)
plt.legend()
plt.ylim(0, 100)
...
plt.plot(hashrates, strategic_pct_pool1, 'ro-', linewidth=2, markersize=6, label='Pool 1 (Honest)')

plt.xlabel('Strategic Pool Hashrate')
plt.ylabel('Reward Percentage (%)')
plt.title('Reward Distribution - Strategic Scenario (Random Fees)')
plt.grid(True, alpha=0.3)
plt.xticks(hashrates)
plt.legend()
plt.ylim(0, 100)

# Subplot 3: Pool 0 comparison (Strategic vs Honest)
plt.subplot(2, 2, 3)
plt.plot(hashrates, honest_pct_pool0, 'b--', linewidth=2, markersize=6, label='All Honest Scenario')
...
plt.title('Pool 0 (Strategic) - Scenario Comparison (Random Fees)')
plt.grid(True, alpha=0.3)
plt.xticks(hashrates)
plt.legend()
plt.ylim(0, 100)

# Subplot 4: Pool 1 comparison (Strategic vs Honest)
plt.subplot(2, 2, 4)
plt.plot(hashrates, honest_pct_pool1, 'r--', linewidth=2, markersize=6, label='All Honest Scenario')
...
plt.title('Pool 1 (Honest) - Scenario Comparison (Random Fees)')
plt.grid(True, alpha=0.3)
plt.xticks(hashrates)
plt.legend()
plt.ylim(0, 100)
...
# Create stacked bars for honest scenario
plt.subplot(1, 2, 1)
plt.bar(x_pos, honest_pct_pool0, width, label='Pool 0 (Strategic)', color='blue', alpha=0.7)
plt.bar(x_pos, honest_pct_pool1, width, bottom=honest_pct_pool0, label='Pool 1 (Honest)', color='red', alpha=0.7)
...
plt.bar(x_pos, strategic_pct_pool1, width, bottom=strategic_pct_pool0, label='Pool 1 (Honest)', color='red', alpha=0.7)
...
honest_streaks = honest_metrics['withholding_streaks']

# Create histogram bins
max_streak = max(max(strategic_streaks) if strategic_streaks else 0, 
                 max(honest_streaks) if honest_streaks else 0)
...
plt.hist(honest_streaks, bins=bins, alpha=0.7, label='All-Honest Scenario', 
         color='blue', edgecolor='black', density=True)

plt.xlabel('Withholding Streak Length (blocks)')
plt.ylabel('Density')
plt.title('Distribution of Withholding Streaks (Random Fees)')
plt.legend()
plt.grid(True, alpha=0.3)

# Add statistics as text
strategic_stats = f"Strategic: {len(strategic_streaks)} streaks, avg={strategic_metrics['avg_withholding_streak']:.1f}"
honest_stats = f"Honest: {len(honest_streaks)} streaks, avg={honest_metrics['avg_withholding_streak']:.1f}"
